...
namespace ycsbc {
    EBFSepDB::EBFSepDB(const char* dbfilename,const char* configPath)
    {
        leveldb::Options options;
        LevelDB_ConfigMod::getInstance().setConfigPath(configPath);
        std::string bloom_filename;
        char *bloom_filename_char;
        string vlogFileName = LevelDB_ConfigMod::getInstance().getVlogFilename();
        ...
        // leveldb::setDirectIOFlag(directIO_flag);
        leveldb::Status status = leveldb::SepDB::Open(options,dbfilename,vlogFileName, &db_);
        if(!status.ok()){
    ...
    bool  EBFSepDB::hasRead = false;
    int EBFSepDB::Read(const string& table, const string& key, const vector< string >* fields, vector< DB::KVPair >& result)
    {
    ...
    int EBFSepDB::Insert(const string& table, const string& key, vector< DB::KVPair >& values)
    {
        leveldb::Status s;
        int count = 0;
        // cout<<key<<endl;
    ...
    int EBFSepDB::Delete(const string& table, const string& key)
    {
        vector<DB::KVPair> values;
        return Insert(table,key,values);
    }

    int EBFSepDB::Scan(const string& table, const string& key, int len, const vector< string >* fields, vector< vector< DB::KVPair > >& result)
    {
        fprintf(stderr,"not implement yet");
        return DB::kOK;
    }

    int EBFSepDB::Update(const string& table, const string& key, vector< DB::KVPair >& values)
    {
        return Insert(table,key,values);
    }
    ...
    void EBFSepDB::printFilterCount()
    {
        int fd[7],i;
        int levels = 0;
        char buf[100];
    ...
    void EBFSepDB::doSomeThing(const char* thing_str)
    {
