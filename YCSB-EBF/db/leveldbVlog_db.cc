...
namespace ycsbc {
    LeveldbVlog::LeveldbVlog(const char* dbfilename,const char* configPath)
    {
        leveldb::Options options;
        LevelDB_ConfigMod::getInstance().setConfigPath(configPath);
        std::string bloom_filename;
        std::string vlog_filename = LevelDB_ConfigMod::getInstance().getVlogFilename();
        ...
        // leveldb::setDirectIOFlag(directIO_flag);
        leveldb::Status status = leveldb::SepDB::Open(options,dbfilename,vlog_filename, &db_);
        if(!status.ok()){
    ...
    bool  LeveldbVlog::hasRead = false;
    int LeveldbVlog::Read(const string& table, const string& key, const vector< string >* fields, vector< DB::KVPair >& result)
    {
    ...
    int LeveldbVlog::Insert(const string& table, const string& key, vector< DB::KVPair >& values)
    {
        leveldb::Status s;
        int count = 0;
        // cout<<key<<endl;
    ...
    int LeveldbVlog::Delete(const string& table, const string& key)
    {
        vector<DB::KVPair> values;
        return Insert(table,key,values);
    }

    int LeveldbVlog::Scan(const string& table, const string& key, int len, const vector< string >* fields, vector< vector< DB::KVPair > >& result)
    {
        fprintf(stderr,"not implement yet");
        return DB::kOK;
    }

    int LeveldbVlog::Update(const string& table, const string& key, vector< DB::KVPair >& values)
    {
        return Insert(table,key,values);
    }

    void LeveldbVlog::Close()
    {
    ...
    void LeveldbVlog::printAccessFreq()
    {
        int fd[7],i;
        int levels = 0;
        char buf[100];
    ...
    void LeveldbVlog::printFilterCount()
    {
        int fd[7],i;
        int levels = 0;
        char buf[100];
    ...
    void LeveldbVlog::doSomeThing(const char* thing_str)
    {
    ...
    void LeveldbVlog::openStatistics(){
        std::string stat_str;
        db_->GetProperty("leveldb.stats",&stat_str);
