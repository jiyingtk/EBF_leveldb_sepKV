...
with open(infilename,'r') as infile:
    ...
    readcachetime = re.findall("read\.cache\.time.*?ave:([0-9.]+)",filetext)[1]
    readcachecnt = re.findall("read\.cache\.time.*?count:([0-9]+)",filetext)[1]
    readnocachetime = re.findall("read\.no\.cache\.time.*?ave:([0-9.]+)",filetext)[1]
    readnocachecnt = re.findall("read\.no\.cache\.time.*?count:([0-9]+)",filetext)[1]
