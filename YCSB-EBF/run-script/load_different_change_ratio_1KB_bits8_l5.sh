...
function __loadLSM(){
    ...
    mv phase_time.txt "$dirname"/"$arrayname"_phase_time"$loadname"
...
level=5
sizeRatio=10
dbfilename="$dbfilename_o"l"$level"s"$sizeRatio"b"$bloombits"_"$arrayname"
...
do
	...
	do
	    ...
	    __loadLSM 20GBtrim_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" "$dirname" "$level"  "$lsmtype" "$bloombits"
